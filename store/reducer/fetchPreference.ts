...
const fetchPreferencesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(api.fetchPreferences.fulfilled, (state, action: PayloadAction<any>) => {
      state.loading = false;
      state.success = true;
      state.message = 'Preferences fetched successfully';
      state.data = action.payload;
    });
    builder.addCase(api.fetchPreferences.rejected, (state, action: PayloadAction<any>) => {
      state.loading = false;
